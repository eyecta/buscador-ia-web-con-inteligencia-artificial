...
function handleSaveAdvertisement() {
    global $message, $error, $dbInstance;
    
    $adId = (int)($_POST['ad_id'] ?? 0);
    $title = trim($_POST['ad_title'] ?? '');
    $content = $_POST['ad_content'] ?? '';
    $position = $_POST['ad_position'] ?? 'top';
    $clickUrl = $_POST['ad_click_url'] ?? '';
    $isActive = isset($_POST['ad_active']) ? 1 : 0;
    ...
    try {
        if ($adId > 0) {
            ...
            $stmt = $dbInstance->conn->prepare("UPDATE advertisements SET title = ?, content = ?, position = ?, click_url = ?, is_active = ? WHERE id = ?");
            $stmt->bindValue(1, $title, SQLITE3_TEXT);
            $stmt->bindValue(2, $content, SQLITE3_TEXT);
            $stmt->bindValue(3, $position, SQLITE3_TEXT);
            $stmt->bindValue(4, $clickUrl, SQLITE3_TEXT);
            $stmt->bindValue(5, $isActive, SQLITE3_INTEGER);
            $stmt->bindValue(6, $adId, SQLITE3_INTEGER);
        } else {
            // Create new advertisement
            $stmt = $dbInstance->conn->prepare("INSERT INTO advertisements (title, content, position, click_url, is_active) VALUES (?, ?, ?, ?, ?)");
            $stmt->bindValue(1, $title, SQLITE3_TEXT);
            $stmt->bindValue(2, $content, SQLITE3_TEXT);
            $stmt->bindValue(3, $position, SQLITE3_TEXT);
            $stmt->bindValue(4, $clickUrl, SQLITE3_TEXT);
        ...
        $error = 'Error al guardar el anuncio: ' . $e->getMessage();
    }
}
...
function handleDeleteAdvertisement() {
    ...
    try {
        $stmt = $dbInstance->conn->prepare("DELETE FROM advertisements WHERE id = ?");
        $stmt->bindValue(1, $adId, SQLITE3_INTEGER);
        $stmt->execute();
...
<div class="max-w-7xl mx-auto">
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <p class="text-gray-600 mb-4">Administra los anuncios que se muestran en la página de resultados de búsqueda</p>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="ad_content" rows="3" 
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                  placeholder="Contenido del anuncio"><?= htmlspecialchars($_POST['ad_content'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="url" name="ad_click_url" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="https://ejemplo.com"
                                   value="<?= htmlspecialchars($_POST['ad_click_url'] ?? '') ?>">
                        </div>
